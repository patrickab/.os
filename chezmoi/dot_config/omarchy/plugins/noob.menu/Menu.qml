...
Item {
  ...
  // root.shell.appLibrary is null for cloned menu plugins (Omarchy 4.0.3 bug); use a local copy instead.
  ...
  // Local stand-in for the shell's AppLibrary service: root.shell.appLibrary
  // comes back null for third-party/cloned menu plugins (Omarchy 4.0.3 bug),
  // so this runs the same DesktopEntries-backed logic directly.
  Item {
    id: localAppLibrary
    ...
    function entrySortKey(entry) {
      return localAppLibrary.entryName(entry).toLowerCase()
    }
    ...
    function entrySearchText(entry) {
      if (!entry) return ""
      return [entry.name, entry.genericName, entry.comment, localAppLibrary.keywordText(entry), entry.id].join(" ").toLowerCase()
    }
    ...
    function words(value) {
      var values = localAppLibrary.wordText(value).split(/[^a-z0-9]+/)
      var result = []
    ...
    function entryAcronym(entry) {
      var values = localAppLibrary.words([entry && entry.name, entry && entry.genericName, localAppLibrary.keywordText(entry), entry && entry.id].join(" "))
      var result = ""
    ...
    function termMatches(entry, term) {
      if (!term) return true
      var name = localAppLibrary.entryName(entry).toLowerCase()
      var id = String((entry && entry.id) || "").toLowerCase()
      var haystack = localAppLibrary.entrySearchText(entry)
      if (name.indexOf(term) >= 0) return true
      if (id.indexOf(term) >= 0) return true
      if (haystack.indexOf(term) >= 0) return true
      return term.length <= 5 && localAppLibrary.entryAcronym(entry).indexOf(term) >= 0
    }

    function allTermsMatch(entry, query) {
      var terms = String(query || "").toLowerCase().trim().split(/\s+/)
      for (var i = 0; i < terms.length; i++) {
        if (terms[i] && !localAppLibrary.termMatches(entry, terms[i])) return false
      }
      return true
    }
    ...
    function fuzzyScore(entry, query) {
      ...
      if (!q) return 0
      if (!localAppLibrary.allTermsMatch(entry, q)) return -1
      var name = localAppLibrary.entryName(entry).toLowerCase()
      var id = String((entry && entry.id) || "").toLowerCase()
      var haystack = localAppLibrary.entrySearchText(entry)
      ...
      var acronym = localAppLibrary.entryAcronym(entry)
    ...
    function isHiddenEntry(entry) {
      ...
      return localAppLibrary.configuredHiddenEntryIds[id] === true || localAppLibrary.desktopHiddenEntryIds[id] === true
    }
    ...
    function sortedEntries(query) {
      ...
      for (var i = 0; i < values.length; i++) {
        ...
        if (localAppLibrary.isHiddenEntry(entry)) continue
        var name = localAppLibrary.entryName(entry)
        if (!name) continue
        var score = localAppLibrary.fuzzyScore(entry, q)
        if (score < 0) continue
        rows.push({ entry: entry, score: score, key: localAppLibrary.entrySortKey(entry), name: name.toLowerCase() })
      }
      rows.sort(function(a, b) {
    ...
    function launch(desktopId, name) {
      ...
      if (!id) return
      localAppLibrary.beginLaunchFeedback(name)
    ...
    function remove(desktopId, name) {
      var id = String(desktopId || "")
      if (!id) return
      Util.execDetached(Util.shellQuote(localAppLibrary.omarchyPath + "/bin/omarchy-remove-launcher-entry") + " " + Util.shellQuote(id) + " " + Util.shellQuote(String(name || id)))
    }
    ...
    function loadConfiguredHides(rawText) {
      ...
      for (var i = 0; i < lines.length; i++) {
        var id = localAppLibrary.normalizeDesktopId(lines[i])
        if (id.length > 0) next[id] = true
      }
      localAppLibrary.configuredHiddenEntryIds = next
    ...
    function loadDesktopHiddenEntries(rawText) {
      ...
      localAppLibrary.desktopHiddenEntryIds = next
      localAppLibrary.appsChanged()
    }
    ...
    function hiddenEntryScanCommand() {
      ...
      var script = localAppLibrary.omarchyPath + "/shell/services/hidden-entries.sh"
    ...
    function beginLaunchFeedback(name) {
      localAppLibrary.launchSerial++
      localAppLibrary.launchToplevelCount = localAppLibrary.toplevelCount()
      localAppLibrary.launchActiveToplevel = ToplevelManager.activeToplevel
      localAppLibrary.launchOsdMessage = "Launching " + String(name || "application") + "…"
      launchDelay.restart()
      launchTimeout.restart()
    }

    function closeLaunchFeedback(serial) {
      if (serial !== localAppLibrary.launchSerial) return
      launchDelay.stop()
      launchTimeout.stop()
      if (localAppLibrary.launchOsdOpen) {
        Quickshell.execDetached(["omarchy-shell", "osd", "close"])
        localAppLibrary.launchOsdOpen = false
      }
    }

    function maybeFinishLaunchFeedback() {
      if (!launchDelay.running && !launchTimeout.running && !localAppLibrary.launchOsdOpen) return
      if (localAppLibrary.toplevelCount() <= localAppLibrary.launchToplevelCount && ToplevelManager.activeToplevel === localAppLibrary.launchActiveToplevel) return
      localAppLibrary.closeLaunchFeedback(localAppLibrary.launchSerial)
    }
    ...
    Process {
      id: hiddenEntryScan
      command: ["bash", "-c", localAppLibrary.hiddenEntryScanCommand()]
      stdout: SplitParser { onRead: function(line) { hiddenEntryOutput.text += line + "\n" } }
      onStarted: hiddenEntryOutput.text = ""
      onExited: localAppLibrary.loadDesktopHiddenEntries(hiddenEntryOutput.text)
    }
    ...
    FileView {
      path: localAppLibrary.omarchyPath + "/default/omarchy/launcher.hides"
      watchChanges: true
      printErrors: false
      onLoaded: localAppLibrary.loadConfiguredHides(text())
      onFileChanged: localAppLibrary.loadConfiguredHides(text())
      onLoadFailed: localAppLibrary.loadConfiguredHides("")
    }

    Connections {
      target: ToplevelManager.toplevels
      function onValuesChanged() { localAppLibrary.maybeFinishLaunchFeedback() }
    }

    Connections {
      target: ToplevelManager
      function onActiveToplevelChanged() { localAppLibrary.maybeFinishLaunchFeedback() }
    }
    ...
    Timer {
      ...
      onTriggered: {
        if (localAppLibrary.toplevelCount() > localAppLibrary.launchToplevelCount || ToplevelManager.activeToplevel !== localAppLibrary.launchActiveToplevel) return
        localAppLibrary.launchOsdOpen = true
        Quickshell.execDetached(["omarchy-shell", "osd", "show", JSON.stringify({ icon: "󱓞", message: localAppLibrary.launchOsdMessage, duration: 0 })])
      }
    }
    ...
    Timer {
      id: launchTimeout
      interval: 15000
      onTriggered: localAppLibrary.closeLaunchFeedback(localAppLibrary.launchSerial)
    }
