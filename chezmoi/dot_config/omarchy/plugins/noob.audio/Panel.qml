...
Panel {
  ...
  readonly property var blanketSounds: [
    { id: "rain", label: "Rain", icon: "☔" },
    { id: "storm", label: "Storm", icon: "ϟ" },
  ...
  property bool blanketExpanded: false
  property string blanketPresetPath: ""
  property var blanketMute: ({})
  property var blanketVolume: ({})
  property bool blanketRestarting: false
  property bool blanketRestartQueued: false

  function blanketMuted(id) { return blanketMute[id] === undefined ? true : blanketMute[id] }
  function blanketSoundVolume(id) { return blanketVolume[id] === undefined ? 0.5 : blanketVolume[id] }
  function parseBlanketDict(text) {
    try { return JSON.parse(String(text || "").trim().replace(/'/g, "\"")) }
    catch (error) { return {} }
  }
  function formatBlanketDict(values, decimal) {
    var parts = []
    for (var key in values)
      parts.push("'" + key + "': " + (decimal ? Number(values[key]).toFixed(4) : (values[key] ? "true" : "false")))
    return "{" + parts.join(", ") + "}"
  }
  function loadBlanketPreset() { blanketPresetProc.running = true }
  function startBlanketRestart() {
    if (!blanketRestartQueued) return
    blanketRestartQueued = false
    blanketRestarting = true
    blanketShutdownProc.running = true
  }
  function applyBlanketSettings() {
    if (!blanketPresetPath) return
    blanketRestartQueued = true
    if (!blanketRestarting) startBlanketRestart()
  }
  function setBlanketMuted(id, value) {
    var next = Object.assign({}, blanketMute)
    next[id] = value
    blanketMute = next
    applyBlanketSettings()
  }
  function setBlanketVolume(id, value) {
    var volumes = Object.assign({}, blanketVolume)
    volumes[id] = value
    blanketVolume = volumes
    if (value > 0 && blanketMuted(id)) {
      var mutes = Object.assign({}, blanketMute)
      mutes[id] = false
      blanketMute = mutes
    }
    applyBlanketSettings()
  }
  function setBlanketPlayback(playing) {
    blanketPlaybackProc.command = playing
      ? ["bash", "-c", "gdbus call --session --dest org.mpris.MediaPlayer2.Blanket --object-path /org/mpris/MediaPlayer2 --method org.mpris.MediaPlayer2.Player.Play >/dev/null 2>&1 || setsid blanket --hidden >/dev/null 2>&1 &"]
      : ["gdbus", "call", "--session", "--dest", "org.mpris.MediaPlayer2.Blanket", "--object-path", "/org/mpris/MediaPlayer2", "--method", "org.mpris.MediaPlayer2.Player.Pause"]
    blanketPlaybackProc.running = true
  }
  ...
  Process {
    id: blanketPresetProc
    command: ["gsettings", "get", "com.rafaelmardojai.Blanket", "active-preset"]
    stdout: StdioCollector {
      waitForEnd: true
      onStreamFinished: {
        var id = String(text || "").trim().replace(/^'|'$/g, "")
        if (!id) return
        root.blanketPresetPath = "/com/rafaelmardojai/Blanket/" + id
        blanketMuteRead.command = ["dconf", "read", root.blanketPresetPath + "/sounds-mute"]
        blanketMuteRead.running = true
        blanketVolumeRead.command = ["dconf", "read", root.blanketPresetPath + "/sounds-volume"]
        blanketVolumeRead.running = true
      }
    }
  }
  Process {
    id: blanketMuteRead
    stdout: StdioCollector { waitForEnd: true; onStreamFinished: root.blanketMute = root.parseBlanketDict(text) }
  }
  Process {
    id: blanketVolumeRead
    stdout: StdioCollector { waitForEnd: true; onStreamFinished: root.blanketVolume = root.parseBlanketDict(text) }
  }
  Process {
    id: blanketShutdownProc
    command: ["bash", "-c", "gdbus call --session --dest org.mpris.MediaPlayer2.Blanket --object-path /org/mpris/MediaPlayer2 --method org.mpris.MediaPlayer2.Quit >/dev/null 2>&1 || true; sleep 1"]
    onExited: {
      blanketMuteWrite.command = ["dconf", "write", root.blanketPresetPath + "/sounds-mute", root.formatBlanketDict(root.blanketMute, false)]
      blanketMuteWrite.running = true
    }
  }
  Process {
    id: blanketMuteWrite
    onExited: {
      blanketVolumeWrite.command = ["dconf", "write", root.blanketPresetPath + "/sounds-volume", root.formatBlanketDict(root.blanketVolume, true)]
      blanketVolumeWrite.running = true
    }
  }
  Process {
    id: blanketVolumeWrite
    onExited: blanketLaunchProc.running = true
  }
  Process {
    id: blanketLaunchProc
    command: ["bash", "-c", "setsid blanket --hidden >/dev/null 2>&1 & sleep 3"]
    onExited: {
      root.blanketRestarting = false
      if (root.blanketRestartQueued) root.startBlanketRestart()
      else root.loadBlanketPreset()
    }
  }
  Process {
    id: blanketPlayToggle
    command: ["bash", "-c", "gdbus call --session --dest org.mpris.MediaPlayer2.Blanket --object-path /org/mpris/MediaPlayer2 --method org.mpris.MediaPlayer2.Player.PlayPause >/dev/null 2>&1 || setsid blanket --hidden >/dev/null 2>&1 &"]
  }
  Process { id: blanketPlaybackProc }
        ...
        Column {
          ...
          Column {
            ...
            Button {
              ...
              foreground: root.bar.foreground
              active: root.blanketExpanded
              bordered: true
              leftAlign: true
              tooltipText: root.blanketExpanded ? "Pause and collapse Blanket" : "Play and expand Blanket"
              onClicked: {
                var expanding = !root.blanketExpanded
                root.blanketExpanded = expanding
                root.setBlanketPlayback(expanding)
              }
            }

            Grid {
              visible: root.blanketExpanded
              width: parent.width
              columns: 2
              ...
              Repeater {
                model: root.blanketSounds
                delegate: Item {
                  required property var modelData
                  width: (parent.width - parent.columnSpacing) / 2
                  height: Style.space(36)
                  readonly property string soundId: modelData.id
                  readonly property bool enabledSound: !root.blanketMuted(soundId)
                  ...
                  Button {
                    ...
                    bordered: true
                    leftAlign: true
                    onClicked: root.setBlanketMuted(soundId, enabledSound)
                  }
                  ...
                  PanelSlider {
                    ...
                    knobSize: Style.space(7)
                    minimum: 0
                    maximum: 1
                    step: 0.05
                    value: root.blanketSoundVolume(soundId)
                    onReleased: function(value) { root.setBlanketVolume(soundId, value) }
                  }
                }
              }
            }
